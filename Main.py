...
class SettingsDialog(QDialog):
    ...
    def initUI(self):
        ...
        info_label.setStyleSheet(f"font-size: 24px; color: {DARK_TEXT_MUTED}; padding: 10px;")  # 글자 크기 확대
        ...
        for combo in [self.jump_combo, self.up_combo, self.down_combo, 
            ...
            combo.setMinimumHeight(60)  # 더 큰 콤보박스
        ...
        # 제스처 설명 레이블 추가
        jump_desc = QLabel("두 손가락을 모으면 점프합니다.")
        up_desc = QLabel("검지 손가락을 펴면 위로 이동합니다.")
        down_desc = QLabel("검지와 중지를 펴면 아래로 이동합니다.")
        left_desc = QLabel("엄지를 펴면 왼쪽으로 이동합니다.")
        right_desc = QLabel("새끼 손가락을 펴면 오른쪽으로 이동합니다.")
        
        # 설명 스타일 지정
        for label in [jump_desc, up_desc, down_desc, left_desc, right_desc]:
            label.setStyleSheet(f"color: {DARK_TEXT_MUTED}; font-style: italic; font-size: 22px;")  # 글자 크기 확대
        
        # 설정 레이아웃에 위젯 추가
        self.settings_layout.addRow(QLabel(f'<span style="color: {DARK_TEXT}; font-size: 24px;">점프:</span>'), self.jump_combo)
        self.settings_layout.addRow("", jump_desc)
        self.settings_layout.addRow(QLabel(f'<span style="color: {DARK_TEXT}; font-size: 24px;">위로:</span>'), self.up_combo)
        self.settings_layout.addRow("", up_desc)
        self.settings_layout.addRow(QLabel(f'<span style="color: {DARK_TEXT}; font-size: 24px;">아래로:</span>'), self.down_combo)
        self.settings_layout.addRow("", down_desc)
        self.settings_layout.addRow(QLabel(f'<span style="color: {DARK_TEXT}; font-size: 24px;">왼쪽:</span>'), self.left_combo)
        self.settings_layout.addRow("", left_desc)
        self.settings_layout.addRow(QLabel(f'<span style="color: {DARK_TEXT}; font-size: 24px;">오른쪽:</span>'), self.right_combo)
        self.settings_layout.addRow("", right_desc)
        
        self.settings_group.setLayout(self.settings_layout)
        
        # 확인 버튼
        close_button = QPushButton("닫기")
        close_button.setMinimumHeight(80)  # 더 큰 버튼
...
class GameControllerGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        menubar.setStyleSheet(f"font-size: 20px; background-color: {DARK_BG_LIGHT}; color: {DARK_TEXT}; border: none;")  
    ...
    def toggle_compact_mode(self, enable_compact=True):
        ...
        if enable_compact:
            ...
            # 각 레이블 크기 조정
            compact_label_style = f"padding: 5px; background-color: {DARK_BG_LIGHT}; border: 1px solid {DARK_BORDER}; border-radius: 4px; font-size: 12px; color: {DARK_ACCENT};"
            for label in [self.jump_label, self.up_label, self.down_label, self.left_label, self.right_label]:
    ...
    def update_key_mapping_display(self):
        """현재 키 매핑 표시 업데이트"""
        # 컨트롤러 현재 매핑
        mapping = self.controller.gesture_mappings
    ...
    def update_frame(self):
        ...
        try:
            ...
            # 디버그 정보 표시 (활성화 상태, 컨트롤러 모드, 감지된 제스처)
            if self.controller_active:
                mode = "키보드" if self.controller.controller_mode == 1 else "마우스"
                self.status_msg.setText(f"상태: 실행중 | 모드: {mode} | " 
                                       f"V: {self.controller.v_dir} H: {self.controller.h_dir} "
                                       f"Jump: {self.controller.jump}")
